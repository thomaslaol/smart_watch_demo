...
#define i2c_write(dev, reg, len, data) i2c_bus_write_bytes(I2C_NUM_0,dev, reg, data, len)   
#define i2c_read(dev, reg, len, data)  i2c_bus_read_bytes(I2C_NUM_0,dev, reg, data, len)  
#define delay_ms(ms) vTaskDelay(pdMS_TO_TICKS(ms))
#define get_ms(p) (*(p) = (uint32_t)xTaskGetTickCount() * portTICK_PERIOD_MS)
