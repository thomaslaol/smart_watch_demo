...
// 读取FIFO状态寄存器（0x7A）
// static void mpu_read_fifo_status(void)
// {
//     uint8_t status;
//     esp_err_t err = i2c_bus_read_bytes(mpu6050_i2c_port, 0x68, 0x7A, &status, 1);
//     if (err != ESP_OK)
//     {
//         ESP_LOGE("FIFO_STATUS", "读取状态寄存器失败");
//         return;
//     }
//     // 状态位含义：
//     // bit7: FIFO溢出；bit6: FIFO满；bit0: FIFO数据就绪
//     ESP_LOGD(TAG, "状态寄存器: 0x%x (溢出: %d, 满: %d, 就绪: %d)",
//              status, (status & 0x80) >> 7, (status & 0x40) >> 6, status & 0x01);
// }
...
int mpu6050_get_step(unsigned long *count)
{
    ...
    // 直接读取计步器当前值（更可靠）
    result = dmp_get_pedometer_step_count(count);
    if (result != 0)
    {
        ESP_LOGE(TAG, "dmp_get_pedometer_step_count 读取失败: %d", result);
        return 1;
    }
    ESP_LOGD(TAG, "步数: %lu", *count);
    return 0;
}
