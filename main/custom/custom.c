...
static void draw_ruler_scale(lv_obj_t *canvas, int current_angle)
{
    ...
    for(int angle = start_angle; angle <= start_angle + 2 * MAX_ANGLE; angle += ANGLE_STEP) {
        ...
        // 跳过超出画布范围的刻度
        if(x < 5 || x >= RULER_CANVAS_WIDTH - 5) continue;
        ...
        if(angle % 30 == 0) {
            ...
            // 绘制方向汉字（在刻度下方）
            // const char* dir_text = get_direction_text(angle);
            // lv_canvas_draw_text(canvas, 
            //                   x - 15,               // X坐标
            //                   RULER_CENTER_Y + line_length/2 + 5,   // 刻度下方
            //                   30,                   // 最大宽度
            //                   &dir_label_dsc,       
            //                   dir_text);
        }
        // 6. 15°倍数但不是30°倍数的刻度也显示度数
        // else if(angle % 15 == 0) {
        //     char text[8];
        //     int display_angle = angle % 360;
        ...
        //     lv_canvas_draw_text(canvas, 
        //                       x - 15, 
        //                       RULER_CENTER_Y - line_length/2 - 15,  // 刻度上方
        //                       30, 
        //                       &label_dsc, 
        //                       text);
        // }
    ...
    // 7. 绘制中心参考线
    // lv_point_t midline_points[2] = {
    //     {5, RULER_CENTER_Y},
    //     {RULER_CANVAS_WIDTH - 5, RULER_CENTER_Y}
    // };
    // lv_canvas_draw_line(canvas, midline_points, 2, &midline_dsc);
}
